...
| `src/grouped/namespace-context.ts` | `src/grouped/namespace-context.ts` | code verbatim; doc-comment examples retargeted from `CcVPC`/`CcVPCProps` to `TfVpc`/`TfVpcConfig`, SPDX header added |
...
| `src/naming.ts` | `src/naming.ts` | `propertyTypeNamesForResource`'s deterministic collision algorithm kept verbatim; every CFN-specific rule (definition-name recovery, `dedupeDefinitionNames`, `moduleNameFor`) removed. awscc's `Cc` prefix has an analogue here — `Tf`, over a stem the group's `stripPrefixes` decides |
