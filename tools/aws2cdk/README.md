...
| `src/naming.ts` | class/interface/nested-type names. Pure, order-independent. |
...
| `src/naming.ts` | `src/naming.ts` | `propertyTypeNamesForResource`'s deterministic collision algorithm kept verbatim; every CFN-specific rule (`Cc` prefix, definition-name recovery, `dedupeDefinitionNames`, `moduleNameFor`) removed and replaced with terraform-type-derived naming |
