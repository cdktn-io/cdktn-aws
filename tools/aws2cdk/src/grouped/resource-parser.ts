...
// Adapted from src/vendored/cdktn/resource-parser.ts — see tools/aws2cdk/README.md.
//
...
//     The *scope* names are still the vendored ones, because `skipped-attributes.ts` keys off
//     `<provider>.<baseName>.<attribute>` full names verbatim (e.g.
//     `aws.quicksight_template.definition`) — get the root scope name wrong and six large aws
//     attribute trees silently stop being collapsed to `any`.
...
class Parser {
  ...
  /**
   * @param providerName the terraform provider name (`aws`) — the root of every scope's
   *   `baseName`, which `skipped-attributes.ts` matches against.
   * @param baseName the vendored parser's `baseName` for this schema entry: the terraform type
   *   with the `<provider>_` prefix removed (`aws_vpc` -> `vpc`, `data_aws_vpc` unchanged),
   *   or `aws_provider` for the provider block itself.
   * @param isProvider whether this is the provider schema. Provider structs get no
   *   OutputReference class (there is nothing to reference), which the emitter reads off
   *   `Struct#isProvider`, which in turn reads it off the attributes' scopes.
   */
  constructor(
    private readonly providerName: string,
    private readonly baseName: string,
    private readonly isProvider: boolean,
  ) {}
  ...
  public parse(schema: Schema): ParsedResource {
    const rootScope = new Scope({
      ...
      parent: this.isProvider
        ? undefined
        : new Scope({ name: this.providerName, isProvider: true }),
    });
  ...
  public renderAttributesForBlock(parentType: Scope, block: Block, path: readonly string[]): AttributeModel[] {
    ...
    for (const [blockTypeName, blockType] of Object.entries(block.block_types || {})) {
      ...
      const blockStruct = this.addStruct(
        [parentType, new Scope({ name: blockTypeName, parent: parentType, isProvider: parentType.isProvider })],
        childPath,
        blockAttributes,
...
/** Parses one schema entry's terraform block into attributes + (temporarily named) structs. */
export function parseResourceAttributes(
  schema: Schema,
  opts: { providerName: string; baseName: string; isProvider?: boolean },
): ParsedResource {
  return new Parser(opts.providerName, opts.baseName, !!opts.isProvider).parse(schema);
}
