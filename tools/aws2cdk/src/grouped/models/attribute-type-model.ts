...
// Adapted from src/vendored/cdktn/models/attribute-type-model.ts — see tools/aws2cdk/README.md.
import { Struct } from "./struct";
import { uppercaseFirst, downcaseFirst } from "../util";
import { resolveStoredClassName } from "./supported-stored-classes";
import { qualify, resourcePrefix } from "../namespace-context";
...
export class StructAttributeTypeModel implements AttributeTypeModel {
  ...
  getStoredClassInitializer(name: string) {
    return `new ${qualify(this.struct.outputReferenceName)}(this, "${name}")`;
  }

  get storedClassType() {
    return qualify(this.struct.name);
  }
  ...
  get toTerraformFunction() {
    return `${downcaseFirst(resourcePrefix())}${this.struct.name}ToTerraform`;
  }

  get toHclTerraformFunction() {
    return `${downcaseFirst(resourcePrefix())}${this.struct.name}ToHclTerraform`;
  }
