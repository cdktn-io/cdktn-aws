...
// Adapted from src/vendored/cdktn/models/struct.ts — see tools/aws2cdk/README.md.
import { AttributeModel } from "./attribute-model";
import { downcaseFirst } from "../util";
export class Struct {
  constructor(
    /** mutable (was readonly): grouped-generate.ts renames every struct once, after the whole
     * schema entry has been parsed — see naming.ts#propertyTypeNamesForResource. Every other getter here reads `this.name` fresh,
     * so renaming after construction is transparent. */
    public name: string,
    ...
    /** terraform attribute path from the resource root, e.g. ['lifecycle_configuration','rules'].
     * Empty for the resource's own ConfigStruct (which isn't part of this path scheme). */
...
export class ConfigStruct extends Struct {
  constructor(
    ...
    /**
     * The cdktn meta-argument interface this config extends, or `""` for none. The vendored
     * generator expressed "none" by branching in the emitter on `resource.isProvider` (a provider
     * has no `count`/`dependsOn`/`lifecycle`); expressing it on the struct instead keeps the
     * grouped `StructEmitter` free of resource-kind knowledge, which matters because it now emits
     * interfaces for four kinds into two different lexical regions.
     */
  ...
  public get extends(): string {
    return this.extendsClass === "" ? "" : ` extends cdktn.${this.extendsClass}`;
  }
}
