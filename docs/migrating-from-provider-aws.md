...
| class | `S3Bucket` | `s3.AwsBucket` |
| config | `S3BucketConfig` | `s3.AwsBucketConfig` |
| data source | `DataAwsS3Bucket` | `s3.DataAwsBucket` |
| ephemeral | `EphemeralAwsLambdaInvocation` | `lambda.EphemeralAwsInvocation` |
| nested block | `S3BucketCorsRule` | `s3.AwsBucket.CorsRuleProperty` |
| its reference | `S3BucketCorsRuleOutputReference` | `s3.AwsBucket.CorsRulePropertyOutputReference` |
| its list | `S3BucketCorsRuleList` | `s3.AwsBucket.CorsRulePropertyList` |
| its mapper | `s3BucketCorsRuleToTerraform` | `s3.awsBucketMapperCorsRulePropertyToTerraform` |
...
* **The class name drops the group's own service tokens**, because the module already carries them:
  `s3.AwsBucket`, not `s3.AwsS3Bucket`. Which tokens count is curated per group, not guessed —
...
* `S3BucketVersioningA` — the resource `aws_s3_bucket_versioning`, so named because
  ...
  `A`. Here they are just `s3.AwsBucketVersioning`, and the block is `s3.AwsBucket.VersioningProperty`.
* Mapper functions in `s3` and `waf` carry a `Mapper` disambiguator
  (`s3.awsBucketMapperCorsRulePropertyToTerraform`), because plain concatenation is not injective
...
* every import form — deep named, deep `* as`, barrel named, barrel `* as`, and the loader
  ...
  its shape and only changes package, because `aws.s3.AwsBucket` is a perfectly good call site;
...
* `package.json`: `@cdktn/provider-aws` becomes `@cdktn/aws@^0.3.0`, in every dependency block that
  ...
  when that range is inside `@cdktn/aws@^0.3.0`; when it is not, nothing in the manifest is touched —
...
| a re-export of a classic binding (`export { S3Bucket };`) | reported — `export { s3.AwsBucket }` is not valid syntax; re-export it by hand |
...
| a `package.json` that already declares `@cdktn/aws` at a range outside `@cdktn/aws@^0.3.0` | reported, and the manifest is left exactly as it was — overwriting an intentional pin, or keeping an incompatible one, are both guesses. Resolve the range by hand and re-run |
...
* **Go**: `github.com/cdktn-io/cdktn-provider-aws-go/aws/vNN/s3bucket` becomes
  `github.com/cdktn-io/cdktn-aws-go/s3`, one module per service group — the Go directory is the
  service name, with no `aws` prefix ([`docs/v030-naming.md`](./v030-naming.md)).
...
`@cdktn/aws` is an alpha at 0.3.0 with no released consumers, and its version does not track the
provider's — `@cdktn/provider-aws@25.3.0` and `@cdktn/aws@0.3.0` bind the same
