...
`TfFunction`, not `AwsLambdaFunction` — and `naming-map.json` maps every terraform type from its
`@cdktn/provider-aws` identity (`lambdaFunction.LambdaFunction`) to its new one. The rule, its gates
and what it deliberately left alone are in
